...
919r+150 params) with act in {none, relu, leaky, gelu, sigmoid}. ``none`` is
a pure 2-layer linear (cannot dead-lock); ``leaky`` (slope 0.01) and ``gelu``
keep a gradient through the negative interval; ``sigmoid`` is the bounded-
saturating control (output always positive, gradient -> 0 in saturation).
...
IN_DIM = 768
N_CLASSES = 150
ACTS = ["none", "relu", "leaky", "gelu", "sigmoid"]
HIDDEN = [64, 128, 256]
...
def main():
    ...
    results = {
        "description": "Activation ablation on the raw-feature MLP probe. Same structure "
                       ...
                       "{none, relu, leaky, gelu, sigmoid}; lr=1e-2, seed 17, 100ep, batch 256, "
                       "wd=0.01, grad_clip=1.0, AdamW. Task 04's ReLU MLP had fc2 bias-free "
                       "(919r); the relu row here has b2 for structure parity (150 extra "
                       "params, 0.13%). sigmoid is the bounded-saturating control.",
